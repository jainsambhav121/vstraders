...
export default function EditBlogPostPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (!firestore || !id) {
      toast({ variant: 'destructive', title: 'Error', description: 'Firestore is not initialized or Post ID is missing.' });
      return;
    }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto grid max-w-4xl flex-1 auto-rows-max gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden items-center gap-2 md:ml-auto md:flex">
            <Button variant="outline" size="sm" type="button" onClick={() => router.push('/dashboard/blog')}>Discard Changes</Button>
